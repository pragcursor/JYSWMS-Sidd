 ...
 * @NModuleScope Public
 */
define(['N/record', 'N/search', 'N/log', 'N/runtime', 'N/https', 'N/file', 'N/format'],
    function (record, search, log, runtime, https, file, format) {
        ...
        function smartBinOrInvTransfer(data, customRecId) {

            var results = [];

            try {
                var fromBin = data.binId;
                var toBin = data.toBin;
                var itemData = data.itemData || [];
                var pickerName = data.userName || data.username || "";
                //var locationIdFromPayload = parseInt(data.location, 10);
                log.error("smartBinOrInvTransfer data", JSON.stringify(data));
                log.error('smartBinOrInvTransfer', 'From Bin: ' + fromBin + ', To Bin: ' + toBin + ', Picker: ' + pickerName);
                ...
                var locationName = data.locationName || "";
                var mappedLocationId = locationNameToId[locationName];
                log.error("Mapped Location ID", mappedLocationId);
                if (!mappedLocationId) {
                    throw new Error('Unknown locationName: "' + locationName + '". Please add it to the locationNameToId map.');
                }
                // get the time for AMERICA_LOS_ANGELES: 'America/Los_Angeles' using netsuite native method
                ...
                // =========================
                // Get REAL bin locations
                // =========================
                var fromInfo = getBinInfo(fromBin);
                var toInfo = getBinInfo(toBin);
                log.error("From Bin Info", JSON.stringify(fromInfo));
                log.error("To Bin Info", JSON.stringify(toInfo));

                if (!fromInfo.locationId || !toInfo.locationId) {
                    throw new Error("Unable to determine bin locations");
                }

                var sameLocation = (parseInt(fromInfo.locationId, 10) == parseInt(toInfo.locationId, 10));

                // =========================
                // FETCH CURRENT BIN QTY
                // =========================
                var existingQuantitiesMap = simplebinSearch(fromBin);

                // =========================
                // STEP 1: INVENTORY ADJUSTMENT (IF NEEDED)
                // =========================
                var adjustments = [];

                itemData.forEach(function (item) {
                    var itemId = item.itemId.toString();
                    var req = Number(item.quantity) || 0;
                    var have = Number(existingQuantitiesMap[itemId]) || 0;
                ...
                if (adjustments.length > 0) {
                    ...
                    adjRec.setValue({
                        fieldId: 'adjlocation',
                        value: sameLocation ? fromInfo.locationId : fromInfo.locationId
                    });
                    ...
                    adjId = adjRec.save({ enableSourcing: true, ignoreMandatoryFields: true });
                }

                // =========================
                // STEP 2A: BIN TRANSFER (same location)
                // =========================
                if (sameLocation) {
                    log.error("Performing BIN TRANSFER (same location)");
                    ...
                    rec.setValue({ fieldId: 'location', value: fromInfo.locationId });
                ...
                // =========================
                // STEP 2B: INVENTORY TRANSFER (different location)
                // =========================
                else {
                    ...
                    rec.setValue({ fieldId: 'subsidiary', value: 1 });
                    rec.setValue({ fieldId: 'location', value: fromInfo.locationId });
                    rec.setValue({ fieldId: 'transferlocation', value: toInfo.locationId });

                    var timeStr = getLATimeString();

                    var memoText =
                        "From Location: " + (data.locationName || fromInfo.locationId) +
                ...
                log.error('smartTransfer FAILED', e);
        ...
        function getBinInfo(binId) {
            var info = { locationId: null, isInactive: false };
            log.error("getBinInfo binId", binId);
            var s = search.create({
                type: "bin",
            ...
            if (r && r.length) {
                info.locationId = r[0].getValue({ name: "location" });
                // var inactive = r[0].getValue({ name: "isinactive" });
                // info.isInactive = (inactive === true || inactive === 'T');
            }

            return info;
        }

        function activateBin(binId) {
            var info = getBinInfo(binId);
            if (info.isInactive) {
                record.submitFields({
                    type: 'bin',
                    id: binId,
        ...
        function simplebinSearch(binId) {
            try {

                var resultMap = {};

                if (!binId) {
                    throw new Error("Missing binId");
                }

                // =========================
                // GET BIN LOCATION (REAL, NO OVERRIDE)
                // =========================
                ...
                var locationId = binLookup.location &&
                    binLookup.location[0] &&
                    binLookup.location[0].value;

                if (!locationId) {
                    throw new Error("Bin has no location: " + binId);
                }

                // =========================
                // SEARCH INVENTORY IN BIN
                // =========================
                var itemSearch = search.create({
                    ...
                    filters: [
                        ...
                        ['binonhand.location', 'anyof', locationId],
                        'AND',
                        ['inventorylocation', 'anyof', locationId] // extra safety
                    ],
                    columns: [
                ...
                pagedData.pageRanges.forEach(function (range) {
                    ...
                    page.data.forEach(function (result) {
                        ...
                        // Only store if qty exists
                        if (qty > 0) {
                            resultMap[itemId] = qty;
                        }
                    });
                });
                ...
                log.error("binSearch ERROR", e.message);
                return {};
            }
        }
        ...
        return {
            ...
            getBinInventoryDetail: getBinInventoryDetail,
            getBins: getBins,
            smartBinOrInvTransfer: smartBinOrInvTransfer,
        };
    });
